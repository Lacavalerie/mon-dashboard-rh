...
# Configuration
st.set_page_config(page_title="RH Cockpit V71", layout="wide", initial_sidebar_state="expanded")
...
# --- GESTION LOGIN (Identique V70) ---
CLIENTS_DB = {
    "admin": {"password": "rh123", "sheet_name": "Dashboard_Data", "company_name": "H&C CONSEIL"},
    "client_a": {"password": "passwordA", "sheet_name": "Dashboard_Client_A", "company_name": "H&C CONSEIL"},
}
...
if not st.session_state['logged_in']:
    ...
    with c2:
        ...
        with st.expander("Infos de démo"): st.write("ID : admin / MDP : rh123")
    st.stop()
...
def load_data(sheet_name):
    try:
        ...
        # Calculs secondaires (Age, Ancienneté, Ecart)
...
# On charge les 6 tables dynamiquement
...
if rh is not None:
    ...
    if selected == "Dashboard":
        ...
        taux_turnover = calculer_turnover(rh_f) 
    ...
    # 5. TEMPS & PROJETS (Base de Gestion des Temps)
    elif selected == "Temps & Projets":
        st.title("⏳ Suivi des Temps & Projets")
        st.info("Cette page est la base de données pour la feuille 'Temps & Projets'.")
    ...
    # 7. GESTION BDD
        ...
        with tab_temps: st.markdown("<div class='card'>", unsafe_allow_html=True); edited_df = st.data_editor(raw_data['Temps & Projets'], num_rows="dynamic"); if st.button("💾 Sauvegarder Temps"): save_data_to_google(edited_df, 'Temps & Projets'); st.markdown("</div>", unsafe_allow_html=True)
