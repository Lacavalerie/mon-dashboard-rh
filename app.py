...
# --- CONFIGURATION ---
st.set_page_config(page_title="H&C Manager Pro V93", layout="wide")
...
# --- SIDEBAR ---
with st.sidebar:
    app_mode = option_menu("ESPACE", ["RH", "Commercial"], icons=['people', 'briefcase'], default_index=1) # On met le CRM par défaut pour tester
    menu = ["Dashboard", "Formation", "Admin RH"] if app_mode == "RH" else ["Pipeline CRM", "Historique Ventes", "Admin Sales"]
...
if app_mode == "Commercial":
    ...
    if selected == "Pipeline CRM":
        st.title("🎯 Pipeline & Prévisions Commerciales")
        ...
        if not df_p.empty:
            ...
            c1.markdown(f"<div class='card'><div class='kpi-val'>{df_p['Montant_N'].sum():,.0f} €</div><div class='kpi-lbl'>Valeur Totale Pipe</div></div>", unsafe_allow_html=True)
            c2.markdown(f"<div class='card'><div class='kpi-val' style='color:#10b981;'>{df_p['Pondéré'].sum():,.0f} €</div><div class='kpi-lbl'>CA Prévisionnel (Pondéré)</div></div>", unsafe_allow_html=True)
            ...
            with col1:
                st.markdown("<div class='card'><h3>Entonnoir de Vente (par Étape)</h3>", unsafe_allow_html=True)
                # On regroupe par Étape pour voir où sont les dossiers
                if 'Étape' in df_p.columns:
                    df_funnel = df_p.groupby('Étape')['Montant_N'].sum().reset_index()
                    # Ordre logique du funnel
                    ordre = ["Prospection", "Qualification", "Proposition", "Négociation"]
                    df_funnel['Étape'] = pd.Categorical(df_funnel['Étape'], categories=ordre, ordered=True)
                    df_funnel = df_funnel.sort_values('Étape')
                    st.plotly_chart(px.funnel(df_funnel, x='Montant_N', y='Étape', color_discrete_sequence=['#38bdf8']), use_container_width=True)
                else:
                    st.info("Ajoutez une colonne 'Étape' dans votre Excel pour voir le funnel.")
                st.markdown("</div>", unsafe_allow_html=True)

            with col2:
                st.markdown("<div class='card'><h3>🚀 Actions Prioritaires</h3>", unsafe_allow_html=True)
                # On affiche les deals > 70% de probabilité qui ne sont pas encore signés
                urgents = df_p[df_p['Prob_N'] >= 0.7].sort_values('Montant_N', ascending=False)
                for _, row in urgents.iterrows():
                    st.markdown(f"""
                    <div class='alert-card'>
                        <b style='color:#38bdf8;'>{row['Nom opportunité']}</b><br/>
                        {row['Client']} — <b>{row['Montant_N']:,.0f} €</b><br/>
                        <small>Probabilité : {row['Prob_N']*100:.0f}%</small>
                    </div>
                    """, unsafe_allow_html=True)
                st.markdown("</div>", unsafe_allow_html=True)

    elif selected == "Historique Ventes":
        st.title("📊 Analyse des Ventes Réalisées")
        if not df_v.empty:
            df_v['Total_HT'] = df_v['Montant HT'].apply(to_num)
            st.plotly_chart(px.bar(df_v, x='Client', y='Total_HT', color='Produit', title="CA par Client et Produit"))
            st.dataframe(df_v, use_container_width=True)

# --- LOGIQUE RH (Reste identique mais simplifié) ---
elif app_mode == "RH":
    st.title("📊 Espace RH")
    st.info("Le dashboard RH est opérationnel. Utilisez le menu latéral pour naviguer.")
